 ...
 * ADC DMA 私有变量
 *------------------------------------------------------------------------------*/
static uint16_t adc_dma_buffer[ADC_CH_NUM] = {0};  /* DMA循环缓冲区 (中断填充)     */
static uint16_t adc_values[ADC_CH_NUM]     = {0};  /* 安全副本 (DMA回调中同步更新) */

/* 全局传感器数据句柄 (外部通过ADC_GetSensorHandle()只读访问) */
static vehicle_sensors_t g_vehicle_sensors = {0};

/*------------------------------------------------------------------------------
 * 内部辅助: 将原始值转换为电池电压
 *------------------------------------------------------------------------------
 * 标定公式: V_bat = raw * (10V / 1128)
 * 当raw=1128时V_bat=10.0V (欠压阈值)
 * 当raw=1354时V_bat≈12.0V (额定电压)
 */
...
/*------------------------------------------------------------------------------
 * 内部辅助: 将原始值转换为转向角度 (-30° ~ +30°)
 *------------------------------------------------------------------------------
 * 传感器输出0~5V, 经电阻分压后ADC读0~3.3V (4095).
 *
 *   区间划分:
 *     [0,       STEER_LEFT_MAX_RAW)  → 左转区, 线性映射到 -30° ~ 0°
 *     [STEER_LEFT_MAX_RAW, STEER_RIGHT_MIN_RAW] → 中位校准窗口, 0°
 *     (STEER_RIGHT_MIN_RAW, STEER_RAW_MAX]      → 右转区, 线性映射到 0° ~ +30°
 */
static float ADC_ConvertToSteeringAngle(uint16_t raw)
{
    if (raw < STEER_LEFT_MAX_RAW)
    {
        /* 左转区: raw 0 → -30°, raw STEER_LEFT_MAX_RAW → 0° */
    ...
    {
        /* 右转区: raw STEER_RIGHT_MIN_RAW → 0°, raw STEER_RAW_MAX → +30° */
...
/**
  * @brief  ADC DMA转换完成回调 (每轮4通道扫描完成时由HAL库调用)
  * @param  hadc: ADC句柄指针
  * @retval None
  * @note   在中断上下文中执行, 仅做快速内存拷贝, 不做浮点运算
  */
...
/**
  ...
  * @param  None
  * @retval None
  * @note   调用此函数后ADC将连续扫描4通道, 通过DMA循环填充缓冲区
  */
...
/**
  ...
  * @param  sensors: 传感器数据结构体指针 (输出句柄, 为NULL时更新全局句柄)
  * @retval None
  * @note   在主循环中调用, 执行浮点转换运算
  */
